...
export class IndicatorsService {
    ...
    async getCulturasPorEstado(): Promise<{ estado: string, culturas: string[] }[]> {
        const result = this.producerRepository
            .createQueryBuilder()
            .select('culturas_por_estado.estado', 'estado')
            .addSelect('jsonb_object_agg(culturas_por_estado.cultura, culturas_por_estado.quantidade)', 'culturas')
            .from(subQuery => {
                return subQuery
